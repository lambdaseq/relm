...
(defn pattern
  "Validates that a string matches the provided regular expression `regex`.
  Passes for nil or empty string."
  [regex & [msg]]
  (fn [v]
    (if (or (nil? v) (and (string? v) (string/blank? v)))
      nil
      (if (re-find regex (str v))
        nil
...
(defn create
  ...
  - `:initial-values` (map, default `{}`): Map of initial field values.
  - `:validators`     (map, default `{}`): Map of field paths to validator or vector of validators.
  ...
  (let [values (:values form-state)
        ...
        (reduce-kv
          (fn [errs path val-fns]
            (let [val (get-in values path)
                  err (some (fn [val-fn] (run-validator val-fn val)) val-fns)]
              (if err
                (assoc errs path err)
                errs)))
          {}
          ...
          (let [res (v-fn values)]
            (when (map? res)
              (reduce-kv
                (fn [errs k msg]
                  (if msg
                    (assoc errs (normalize-path k) msg)
                    errs))
                {}
                res))))
...
(defn validate-field
  ...
  (let [norm-p (normalize-path path)
        val-fns (get (:validators form-state) norm-p)
        val (get-in (:values form-state) norm-p)
        err (when (seq val-fns)
              (some (fn [val-fn] (run-validator val-fn val)) val-fns))
        custom-err (when-let [v-fn (:validate-fn form-state)]
                     (let [res (v-fn (:values form-state))]
...
(defn touch-all
  ...
  [form-state]
  (let [val-paths (collect-all-paths (:values form-state))
        init-paths (collect-all-paths (:initial-values form-state))
        validator-paths (keys (:validators form-state))
        all-paths (into (set validator-paths) (concat val-paths init-paths))]
...
(defn value
  "Retrieves the current value of a field at `path`, returning `default-val` if nil."
  ([form-state path]
   (value form-state path nil))
  ([form-state path default-val]
   (get-in (:values form-state) (normalize-path path) default-val)))
...
(defn values
  "Returns the complete `:values` map from `form-state`."
  [form-state]
  (or (:values form-state) {}))

(defn initial-values
  "Returns the complete `:initial-values` map from `form-state`."
  [form-state]
  (or (:initial-values form-state) {}))
...
(defn- register-attrs
  ...
  (let [is-checkbox? (= (:type opts) "checkbox")
        default-val (if (contains? opts :default)
                      (:default opts)
                      (if is-checkbox? false ""))
        val (value form-state path default-val)
        base-attrs (cond-> {:on (if is-checkbox?
                                  {:change (on-change form-k path)
                                   :blur   (on-blur form-k path)}
                                  {:input  (on-change form-k path)
                                   :change (on-change form-k path)
                                   :blur   (on-blur form-k path)})}
                     is-checkbox? (assoc :checked (boolean val))
                     (not is-checkbox?) (assoc :value val)
                     (:type opts) (assoc :type (:type opts))
                     (:required opts) (assoc :required true)
                     (some? (:min opts)) (assoc :min (:min opts))
                     (some? (:max opts)) (assoc :max (:max opts))
                     (some? (or (:min-length opts) (:minlength opts))) (assoc :minlength (or (:min-length opts) (:minlength opts)))
                     (some? (or (:max-length opts) (:maxlength opts))) (assoc :maxlength (or (:max-length opts) (:maxlength opts)))
                     (some? (:pattern opts)) (assoc :pattern (if (instance? #?(:clj java.util.regex.Pattern :cljs js/RegExp) (:pattern opts))
                                                               (str (:pattern opts))
                                                               (:pattern opts))))
        extra-attrs (dissoc opts :default :required :min :max :min-length :max-length :minlength :maxlength :pattern :type)]
    (merge base-attrs extra-attrs)))

(defn register
  "Generates standard Hiccup input attributes and event handlers for a form field.
   ...
   - `opts`: (Optional) Map of options and HTML attributes:
     - `:type`        - Input type (\"text\", \"email\", \"password\", \"number\", \"checkbox\", etc.)
     - `:default`     - Fallback default value if not in form state
     - `:required`    - If truthy, sets `:required true`
     - `:min`         - Sets `:min` attribute
     - `:max`         - Sets `:max` attribute
     - `:min-length` / `:minlength` - Sets `:minlength` attribute
     - `:max-length` / `:maxlength` - Sets `:maxlength` attribute
     - `:pattern`     - Sets `:pattern` attribute
     - Any additional HTML attributes (e.g. `:placeholder`, `:autocomplete`, `:disabled`)
   
   Usage:
     (register form :email)
     (register form :email {:type \"email\" :required true})
     (register form [:profile :age] {:type \"number\" :min 18})
...
(defmethod relm/update ::submit
  ...
  (let [[_ form-key {:keys [on-submit on-invalid validate focus-error?]
        ...
        extra-errors (when (fn? validate) (validate (:values validated-form)))
    ...
    (if is-valid?
      (let [submitting-form (submit-start final-form)
            ...
            values (:values submitting-form)
            effects (cond
                      (nil? on-submit) []
                      (fn? on-submit)
                      (let [res (on-submit values)]
                        (if (relm/vector-of-vectors? res) res [[:dispatch res]]))
                      (relm/vector-of-vectors? on-submit)
                      on-submit
                      (vector? on-submit)
                      [[:dispatch (conj on-submit values)]]
                      :else [])]
